...
def main(run_etl: bool = True, run_model: bool = True) -> None:
    ...
    try:
        bronze_results, silver_results, gold_results = {}, {}, {}
        ...
        if run_etl:
            ...
            log.info("Skipping ETL stages (Bronze, Silver, Gold).")
        ...
        if run_model and latent_results.get("status") != "failed":
            log.info("  Latent Demand Model: ✓ Trained")
        elif not run_model:
            log.info("  Latent Demand Model: ⏭ Skipped")
        log.info("=" * 55)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Data pipeline and/or Model training.")
    parser.add_argument("--skip-etl", action="store_true", help="Skip the Bronze, Silver, and Gold ETL stages.")
    parser.add_argument("--skip-model", action="store_true", help="Skip the Latent Demand Model training.")
