...
def write_parquet(df: pd.DataFrame, base_dir: Path, table_name: str, layer: str) -> int:
    ...
    data.to_parquet(parquet_path, index=False, compression=config.parquet_compression, engine="pyarrow")
    log.info(
...
def write_rejects(
    ...
    if "_reject_reason" not in data.columns:
        data["_reject_reason"] = reject_reason if reject_reason is not None else "unknown"
